...
class OOSAnalyzer(Module):
    def configure(self):
        ...
        self.timetoplot=60
        self.final_delay=0
        self.testdf=pd.DataFrame(columns=['Time','DOMnumber', 'deltaT'])        
    ...
    def process(self,blob):
        ...
        if Time == 0.1:
            print("timeslice (?) number",self.counter)
            print ('TSCounter ', TSCounter)
        #print("timestamp",TSindex)
        #print("time",Time)
       ...
       # print('dom1 ',tshits[tshits.dom_id==806451575])
    ...
    #take all the chosen-DOM1-PMT hits' times and calculate dt with all chosen DOM-PMT hits' times, doing this for all DOMs
            ...
            for i in self.orderedDOM[2:]:
                ...
                self.plot_delays(str(Dom_numbers),value)
    ...
    def plot_delays(self,dom,delays):
        fig,ax = plt.subplots()
        x_entries,binning,_ = plt.hist(delays,bins=100,density=0,alpha=0.5,color='black')
