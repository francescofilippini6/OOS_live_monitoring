...
#            print("New Run ",tmch_data.run,' for DOM ',self.Dom_id_name[str(dom_id)]," started at ",self.start_of_run_dom[dom_id],'\n')
    ...
    #---------------------------------------------------------------- IF LAST CHECK --> INTERVAL IS BETWEEN PREVIOUS CHECK AND END OF RUN
        if end_run == 1:
    #        print("controllo i pacchetti del dom prima della fine run",self.Dom_id_name[str(dom_id)])
    #        print('time of current final check (end of run) ',self.end_of_run_dom[dom_id], 'time of previous check ', self.time_of_previous_check[dom_id], 'true interval = ', self.end_of_run_dom[dom_id]-self.time_of_previous_check[dom_id])
    ...
    #        print("controllo i pacchetti del dom ", self.Dom_id_name[str(dom_id)])
    #        print('time of current check ',current_time, 'time of previous check ', self.time_of_previous_check[dom_id], 'true interval = ', current_time-self.time_of_previous_check[dom_id])
