...
class OOSAnalyzer(Module):
    def configure(self):
        ...
        self.delay_detx=self.get("filee")
    ...
    def process(self,blob):
        ...
        if Time==0.1 and TSindex%60==0:
            ...
            delays_detx=self.detx_file_reader()
            now = datetime.now()
            delay_counter=1
            for i in self.orderedDOM[1:]:
                ...
                delay_over_TS=[]
                #print(delays_detx[delay_counter])
                for t in pmt1_3.time:
                    for tt in pmt1_dtype.time:
                        deltat=tt-t-float(delays_detx[delay_counter])
    ...
    def detx_file_reader(self):
        detx = self.delay_detx
        delay_found=[]
